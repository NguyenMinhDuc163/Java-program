...
class Student {
    ...
    public double getTotal() {
        return total + prior.get(idx.substring(0,3));
    }
    ...
    public String toString(){
        double p = prior.get(idx.substring(0,3));
        return String.format("%s %s %s %s %s", idx, name, stdPoint(p),stdPoint(total + p), status);
    }
}
...
public class Main {
    public static void main(String[] args) {
        ...
        students.sort((x, y) -> Double.compare(y.getTotal(), x.getTotal()));
